...
Conversion picks a converter based on extension (and, for `.vtt`, on the detected dialect), prepends a properties block plus `# {meetingName}`, creates `outputFolder` if missing, and writes `{outputFolder}/{noteName}.md` where `noteName` is `buildNoteFileName(meetingName, meetingTime, fileNameOrder, fileNameDateFormat)`. An existing note is overwritten only when its `source` property names the same transcript; otherwise the meeting's `HH-mm-ss` is appended to the name so a different meeting is never replaced.

VTT parsing runs through one shared reader, `parseVttCues`, which returns `{ offsetMs?: number; speaker?: string; text: string }[]` — dropping the header, `NOTE` blocks and cue identifiers (numeric or Teams' `{guid}/{n}-{n}`), handling both `HH:MM:SS.mmm` and `MM:SS.mmm` timestamps, lifting `<v Speaker>` voice tags into `speaker`, and stripping remaining inline tags. `detectVttDialect` then picks the renderer: `convertVttToMarkdown` emits `- **[{moment(fileCreationTime + offsetMs).format(timeFormat)}]** {text}` (plain bullet if `timeFormat` is empty) for caption-style files, while `convertTeamsVttToMarkdown` merges consecutive cues from one speaker and emits `[{speaker}] {time}` followed by the text — the same shape `convertTxtToMarkdown` produces for Zoom transcripts. Both speaker layouts hardcode `YYYY-MM-DD HH:mm:ss` and ignore the `timeFormat` setting, which applies to the bulleted output only.
